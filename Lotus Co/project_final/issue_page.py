...
issue_pic = tk.PhotoImage(file=r"assests\issue_book.png")

heading = ttk.Label(
    issue,
    font=('Poiret One', 40),
    text="Get Books from Lotus Co",
    image=issue_pic,
    compound='top',
)
...
def confirmIssue():
    if issuingBook.get() == "Select any one..." or int(duration.get()) == 0 or emailId.get() == "":
        subprocess.call(r'python popups\blank_error.py')
        issuingBook.set('Select any one...')
        duration.set('0')
        emailId.set("")

    elif "@gmail.com" not in emailId.get():
        subprocess.call(r'python popups\mail_error.py')
        emailId.set("")

    else:
        subprocess.call(r'python popups\issued_book.py')
        ...
        subprocess.call(r'python popups\issued_info.py')
