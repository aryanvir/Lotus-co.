...
add_pic = tk.PhotoImage(file=r"assests\added_icon.png")

heading = ttk.Label(
    add,
    font=('Poiret One', 40),
    text="Add Books to Lotus Co",
    image=add_pic,
    compound='top',
)
...
def confirmAdd():
    ...
    if authorName.get() == "" or bookName.get() == "":
        subprocess.call(r'python project_final\popups\blank_error.py')
        bookName.set("")
        authorName.set("")

    elif bookName.get() in booknames:
        subprocess.call(r'python project_final\popups\exists_error.py')
        bookName.set("")
        authorName.set("")

    else:
        subprocess.call(r'python project_final\popups\added_info.py')
